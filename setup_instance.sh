...
export securityGroupId=$(aws ec2 create-security-group --group-name $name-security-group --description "SG for aws-p2 machine" --vpc-id $vpcId --query 'GroupId' --output text)
# ssh
...
# install packages to new machine
ssh -i ~/.ssh/aws-key-$name.pem ubuntu@$instanceUrl 'bash -s' < install_packages.sh
#ssh -i ~/.ssh/aws-key-aws-p2.pem ubuntu@ec2-35-163-66-27.us-west-2.compute.amazonaws.com 'bash -s' < install_packages.sh
